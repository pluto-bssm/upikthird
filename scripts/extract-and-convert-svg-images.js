...
const svgs = ['Domitory.svg','MakeSchool.svg','Humor.svg','Humors.svg','School.svg'];
...
svgs.forEach((file) => {
  ...
  const dataUriMatch = svg.match(/data:image\/png;base64,([A-Za-z0-9+/=\n\r]+)/);
  if (dataUriMatch) {
    ...
    svg = svg.replace(/xlink:href="data:image\/png;base64,[^"]+"/, `href="/svg/images/${imgName}"`);
  } else {
